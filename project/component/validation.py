...
class DataValidation:
    ...
    def is_train_test_file_exists(self):
        try:
            ...
            if not is_available:
                ...
                message = f"Training: {train_file} or Testing: {test_file} is not present"
    ...
    def validate_dataset_schema(self):
        try:
            ...
            train_data,test_data = self.get_train_and_test_df()
            schema_path = self.data_validation_config.schema_file_path
            schema = read_yaml(schema_path)
            
            schema_columns = schema[DATASET_SCHEMA_COLUMNS_KEY]
            
            for col in train_data.columns.to_list():
                if schema_columns[col] == train_data[col].dtype:
